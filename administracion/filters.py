...
from administracion.models import Viajante, Transporte, CondicionDePago, MedioDePago, Moneda, GrupoContactos
...
from .models import Departamento, Deposito, GrupoEconomico
...
def grupocontactos_filtrar(query_dict):
    buscar = query_dict.GET.get('buscar')
    items = get_opcion_paginado(query_dict)
    modo = query_dict.GET.get('modo')
    filtrado = GrupoContactos.objects.all()
...
def grupoeconomico_filtrar(query_dict):
    buscar = query_dict.GET.get('buscar')
    items = get_opcion_paginado(query_dict)
    modo = query_dict.GET.get('modo')
    filtrado = GrupoEconomico.objects.all()
